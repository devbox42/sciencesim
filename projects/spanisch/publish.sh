#!/bin/bash
# ═══════════════════════════════════════════════════════════════
# PUBLISH SPANISCH: Veröffentlicht Spanisch-Materialien auf GitHub
#
# Verwendung (aus dem spanisch-Ordner):
...
echo ""
echo -e "${BLUE}═══════════════════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  Spanisch → GitHub ($REPO)${NC}"
echo -e "${BLUE}═══════════════════════════════════════════════════════════════${NC}"
echo ""

# Ohne Argument: Sync alle online vorhandenen Dateien
if [ -z "$1" ]; then
    echo "Hole online vorhandene Spanisch-Dateien..."
    echo ""

    REMOTE_FILES=$(gh api "repos/$REPO/git/trees/main?recursive=1" \
        --jq '.tree[] | select(.type=="blob") | select(.path | startswith("spanisch/")) | .path' 2>/dev/null)

    if [ -z "$REMOTE_FILES" ]; then
        echo -e "${YELLOW}Keine Spanisch-Dateien online gefunden.${NC}"
        exit 0
    fi

    UPDATED=0
    while IFS= read -r remote_path; do
        # Remote: spanisch/kl08/... -> Local: kl08/...
        local_relative="${remote_path#spanisch/}"
