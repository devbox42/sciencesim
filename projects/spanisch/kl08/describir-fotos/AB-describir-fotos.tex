...
\textbf{Estar:} \luecke{1.5cm}, estás, \luecke{1.5cm}, estamos, estáis, \luecke{1.5cm}

\vspace{6pt}

\textbf{Beispiel:}

\es{La chica \luecke{1.2cm} \luecke{2cm}.}
...
\midrule
caminar \de{laufen} & \luecke{2.5cm} \\[4pt]
pasear \de{spazieren} & \luecke{2.5cm} \\[4pt]
mirar \de{schauen} & \luecke{2.5cm} \\[4pt]
hablar \de{sprechen} & \luecke{2.5cm} \\[4pt]
comer \de{essen} & \luecke{2.5cm} \\[4pt]
beber \de{trinken} & \luecke{2.5cm} \\[4pt]
leer \de{lesen} & \luecke{2.5cm} \\[4pt]
escribir \de{schreiben} & \luecke{2.5cm} \\[4pt]
jugar \de{spielen} & \luecke{2.5cm} \\[4pt]
correr \de{rennen} & \luecke{2.5cm} \\[4pt]
reír \de{lachen} & \luecke{2.5cm} \\[4pt]
dormir \de{schlafen} & \luecke{2.5cm} \\
\bottomrule
\end{tabular}
...
\begin{teil}
    \item El hombre está \luecke{2.2cm} en el banco.\\
          \de{Der Mann sitzt auf der Bank.}
    \item La mujer está \luecke{2.2cm} en la pared.\\
          \de{Die Frau lehnt an der Wand.}
    \item Los niños están \luecke{2.2cm} en el césped.\\
          \de{Die Kinder liegen auf dem Rasen.}
    \item Mi padre está \luecke{2.2cm} delante de la tienda.\\
...
\begin{tikzpicture}[remember picture, overlay]
    ...
    \node[anchor=west, text=white, font=\sanstext\small\bfseries]
        at ([xshift=1.4cm, yshift=-0.35cm]current page.north west) {\thema{} – Fortsetzung};
\end{tikzpicture}
...
\es{En la foto veo un parque en una ciudad. En primer plano hay un banco donde una mujer está sentada. Ella está leyendo un libro. A la izquierda, un hombre está paseando con su perro. Al fondo se ven varios edificios altos. En el centro de la imagen hay una fuente bonita. A la derecha, dos niños están jugando en el césped.}
\end{vokabelbox}
...
{\small
\begin{multicols}{2}
$\square$ Bildaufbau verwendet (primer plano, al fondo...)\\
$\square$ Presente continuo korrekt (estar + gerundio)\\
$\square$ Positionen beschrieben (de pie, sentado...)
\columnbreak
$\square$ Personen beschrieben (pelo, ojos, alto...)\\
$\square$ Orte/Gegenstände benannt\\
$\square$ Redemittel genutzt (En la foto veo...)
\end{multicols}
}
\end{checklistbox}
