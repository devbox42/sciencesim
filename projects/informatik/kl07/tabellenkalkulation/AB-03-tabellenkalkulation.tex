% ═══════════════════════════════════════════════════════════════
% ARBEITSBLATT: Tabellenkalkulation Grundlagen
...
\begin{lueckenkasten}
Eine \textbf{Zelle} ist \luecke{6cm}

\vspace{0.3em}
in das man \luecke{4cm} eingeben kann.
\end{lueckenkasten}
...
\textbf{Spalte} & verläuft \luecke{3cm} (↓) \\[0.5em]
                & wird mit \luecke{3cm} bezeichnet (A, B, C, ...) \\[0.8em]
\textbf{Zeile}  & verläuft \luecke{3cm} (→) \\[0.5em]
                & wird mit \luecke{3cm} bezeichnet (1, 2, 3, ...)
\end{tabular}

\vspace{0.5em}
\textit{Merkhilfe:} \textbf{S}palte = \textbf{S}\luecke{2cm}
\end{lueckenkasten}
...
\begin{lueckenkasten}
Formeln beginnen \textbf{immer} mit dem Zeichen: \luecke{1.5cm}

\vspace{0.3em}
Beispiel: \texttt{\luecke{1cm}B2*C2}
\end{lueckenkasten}
...
$\times$ & \luecke{1cm} & Multiplikation \\[0.3em]
$\div$ & \luecke{1cm} & Division \\
\bottomrule
\end{tabular}
\end{center}
\end{lueckenkasten}
...
\begin{lueckenkasten}
Die Formel \texttt{=SUMME(B1:B4)} addiert alle Zellen von \luecke{1.5cm} bis \luecke{1.5cm}.

\vspace{0.3em}
Der \textbf{Doppelpunkt} \texttt{:} bedeutet: \luecke{3cm}
\end{lueckenkasten}
...
a) Zelle mit 0,80: \luecke{2cm} & b) Zelle mit 8: \luecke{2cm} & c) Zelle mit Saft: \luecke{2cm}
\end{tabular}
...
a) \texttt{=A1+B1} & $\square$ addiert A1 und B1 \quad $\square$ multipliziert A1 und B1 \\[0.5em]
b) \texttt{=C2*D2} & $\square$ addiert C2 und D2 \quad $\square$ multipliziert C2 und D2 \\[0.5em]
c) \texttt{=SUMME(B1:B5)} & $\square$ addiert nur B1 und B5 \quad $\square$ addiert B1 bis B5 (alle)
\end{tabular}
...
a) \texttt{B2*C2} & Fehler: \luecke{5cm} \\[0.5em]
b) \texttt{=SUMME B1:B4} & Fehler: \luecke{5cm}
\end{tabular}
...
\begin{lueckenkasten}
Mit dem \textbf{Aufzieh-Trick} kann man Formeln automatisch \luecke{3cm}.

\vspace{0.3em}
Der \textbf{grüne Punkt} befindet sich \luecke{4cm} in der Zelle.

\vspace{0.3em}
Beim Aufziehen wird die Formel automatisch angepasst:

\texttt{=B2*C2} $\rightarrow$ \texttt{=B\luecke{0.5cm}*C\luecke{0.5cm}} $\rightarrow$ \texttt{=B\luecke{0.5cm}*C\luecke{0.5cm}}
\end{lueckenkasten}
...
\fbox{\phantom{0}} & Den grünen Punkt nach unten ziehen (z.B. bis D6) \\[0.4em]
\fbox{\phantom{0}} & Die erste Formel eingeben (z.B. \texttt{=B2*C2} in D2) \\[0.4em]
\fbox{\phantom{0}} & Die Zelle mit der Formel anklicken (z.B. D2) \\[0.4em]
\fbox{\phantom{0}} & Enter drücken
\end{tabular}
...
a) Was steht dann in D3? & \texttt{=}\luecke{3cm} \\[0.5em]
b) Was steht dann in D5? & \texttt{=}\luecke{3cm}
\end{tabular}
