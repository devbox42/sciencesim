% ═══════════════════════════════════════════════════════════════
% ARBEITSBLATT: Der Gleichstrommotor (Kl. 9, DS 09a)
% KOMPAKT-VERSION für 40-Min-Einheit
% ═══════════════════════════════════════════════════════════════
...
\vspace{2mm}
\begin{tabular}{@{}r@{ = }l@{}}
(A) & \luecke{2.2cm} \\[0.25em]
(B) & \luecke{2.2cm} \\[0.25em]
(C) & \luecke{2.2cm} \\[0.25em]
(D) & \luecke{2.2cm} \\[0.25em]
(E) & \luecke{2.2cm} \\
\end{tabular}
\end{minipage}
\hfill
...
\begin{tikzpicture}
    ...
    \begin{scope}[x={(img.south east)}, y={(img.north west)}]
        % Weißer Patch über original + und - (links+unten verschoben)
        \fill[white] (-0.04,0.30) rectangle (0.06,0.54);
        % Vertauschte Pole: - oben (wo + war), + unten (wo - war)
...
Der stromdurchflossene Rotor ist selbst ein \luecke{3cm}!
...
\begin{itemize}[noitemsep, leftmargin=*]
    \item N--S: \luecke{2.5cm}
    \item N--N oder S--S: \luecke{2.5cm}
\end{itemize}
\end{minipage}
\hfill
\begin{minipage}{0.48\textwidth}
\textbf{Warum dreht sich der Rotor?}\\
Der \textcolor{positiv}{N}-Pol des Rotors wird vom \textcolor{negativ}{S}-Pol des Stators \luecke{2.5cm}.
\end{minipage}
...
\textbf{Das Problem:}\\
Der N-Pol des Rotors wird zum S-Pol des Stators gezogen -- und bleibt dort \luecke{2.5cm}!\\
{\small (nach kurzem Pendeln)}

\vspace{2mm}
\textbf{Die Lösung -- Der Kommutator:}\\
Der Kommutator \luecke{4cm} die Stromrichtung im Rotor genau im richtigen Moment.\\
$\Rightarrow$ Die Pole des Rotor-Elektromagneten werden \luecke{2.5cm}.\\
$\Rightarrow$ Jetzt wird er wieder \luecke{2.5cm} und dreht weiter!
...
\begin{tikzpicture}[node distance=0.8cm, every node/.style={font=\small}]
    \node[draw, fill=hellgrau, minimum width=2.2cm, minimum height=0.8cm] (ein) {\luecke{1.8cm}};
    \node[right=of ein, font=\Large] (pfeil) {$\rightarrow$};
    \node[draw, fill=hellgrau, minimum width=2.2cm, minimum height=0.8cm, right=of pfeil] (aus) {\luecke{1.8cm}};
...
\hline
Spannung umpolen (+ und $-$ vertauschen) & $\square$ Ja \quad $\square$ Nein \\
\hline
Magnetfeld umpolen (N und S vertauschen) & $\square$ Ja \quad $\square$ Nein \\
\hline
\textbf{Beides gleichzeitig} ändern & $\square$ Ja \quad $\square$ Nein \\
\hline
\end{tabularx}
