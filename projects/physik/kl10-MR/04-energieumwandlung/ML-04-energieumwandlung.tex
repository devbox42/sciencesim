% ═══════════════════════════════════════════════════════════════
% ML-04: Energieumwandlung & Erhaltungssatz – MUSTERLÖSUNG
% Physik Klasse 10 MR
% ═══════════════════════════════════════════════════════════════
...
\textbf{Lageenergie} $E_\text{pot}$ & Energie aufgrund der \lsg{Höhe / Lage} eines Körpers \\[8pt]
\textbf{Bewegungsenergie} $E_\text{kin}$ & Energie aufgrund der \lsg{Geschwindigkeit / Bewegung} eines Körpers \\[8pt]
\textbf{Thermische Energie} $E_\text{therm}$ & Energie aufgrund der \lsg{Bewegung / Temperatur} der Teilchen \\[8pt]
\textbf{Elektrische Energie} $E_\text{el}$ & Energie durch bewegte \lsg{Ladungen / Elektronen} \\[8pt]
\textbf{Chemische Energie} $E_\text{chem}$ & In \lsg{Stoffen / Bindungen} gespeicherte Energie
\end{tabular}
\end{merkkasten}
...
\vspace{4pt}
Energie kann nicht erzeugt oder \lsg{vernichtet} werden.

Sie wird nur von einer Form in eine andere \lsg{umgewandelt}.

\vspace{6pt}
\textbf{Gültigkeitsbedingung:} Der Erhaltungssatz gilt nur in einem \lsg{geschlossenen / abgeschlossenen} System
...
\begin{tikzpicture}[scale=0.9]
    ...
    \begin{scope}[shift={(1.2,0)}]
        ...
        \node[loesung] at (-1.2,-0.1) {\footnotesize $E_\text{pot}$: \textbf{max}};
        \node[loesung] at (-1.2,-0.5) {\footnotesize $E_\text{kin}$: \textbf{0}};
    \end{scope}
    ...
    \begin{scope}[shift={(4,0)}]
        ...
        \node[loesung] at (0,-0.3) {\footnotesize $E_\text{pot}$: \textbf{0}};
        \node[loesung] at (0,-0.7) {\footnotesize $E_\text{kin}$: \textbf{max}};
    \end{scope}
...
\begin{tikzpicture}[
    ...
    \node[box] (epot1) at (0,0) {\lsg{$E_\text{pot}$}};
    \node[box] (ekin) at (3.5,0) {\lsg{$E_\text{kin}$}};
    \node[box] (epot2) at (7,0) {\lsg{$E_\text{pot}$}};
...
\vspace{4pt}
\textbf{In der Realität:} Ein Teil der Energie wird durch \lsg{Reibung} in \lsg{Wärme / $E_\text{therm}$} umgewandelt.

Deshalb schwingt das Pendel immer \lsg{niedriger / weniger} und kommt schließlich zur Ruhe.
\end{merkkasten}
...
\vspace{4pt}
Der Wirkungsgrad $\eta$ (eta) gibt an, welcher Anteil der zugeführten Energie \lsg{genutzt / nutzbar} wird.

\textit{Alternativ kann statt der Energie auch die Leistung $P$ eingesetzt werden.}

$\eta$ liegt immer zwischen \lsg{0} (0\%) und \lsg{1} (100\%).

\vspace{6pt}
\textbf{Beispiele:}
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
\item Elektromotor: $\eta \approx$ \lsg{90\% (0,90)}
\item LED: $\eta \approx$ \lsg{40\% (0,40)}
\item Glühbirne: $\eta \approx$ \lsg{5\% (0,05)}
\end{itemize}
...
\begin{tikzpicture}[
    ...
    \node[box] (a) at (0,0) {\lsg{$E_\text{pot}$}};
    \node[box] (b) at (4.5,0) {\lsg{$E_\text{kin}$}};
    \draw[arr] (a) -- (b);
...
\begin{tikzpicture}[
    ...
    \node[box] (a) at (0,0) {\lsg{$E_\text{kin}$}};
    \node[box] (b) at (4.5,0) {\lsg{$E_\text{therm}$}};
    \draw[arr] (a) -- (b);
...
\lsg{$E_\text{pot} = 2\,\text{kg} \cdot 10\,\frac{\text{m}}{\text{s}^2} \cdot 0{,}5\,\text{m} = 10\,\text{J}$}

\textbf{Ergebnis:} $E_\text{pot} = $ \lsg{10 J}
...
\lsg{Nach dem Energieerhaltungssatz wird die gesamte Lageenergie in Bewegungsenergie umgewandelt.}

\textbf{Ergebnis:} $E_\text{kin} = $ \lsg{10 J}
\end{teil}
...
\lsg{$v = \sqrt{\frac{2 \cdot 10\,\text{J}}{2\,\text{kg}}} = \sqrt{\frac{20}{2}\,\frac{\text{m}^2}{\text{s}^2}} = \sqrt{10\,\frac{\text{m}^2}{\text{s}^2}} \approx 3{,}16\,\frac{\text{m}}{\text{s}}$}

\textbf{Ergebnis:} $v = $ \lsg{$\sqrt{10}\,\frac{\text{m}}{\text{s}} \approx 3{,}2\,\frac{\text{m}}{\text{s}}$}
...
\vspace{4pt}
\textbf{Hinweis:} Nutze die Energieerhaltung: $E_\text{pot} = E_\text{kin}$ \quad $\Rightarrow$ \quad $m \cdot g \cdot h = \frac{1}{2} m \cdot v^2$

\vspace{6pt}
\textbf{Rechnung:}

\lsg{$m \cdot g \cdot h = \frac{1}{2} m \cdot v^2$ \quad (m kürzt sich heraus)}

\lsg{$g \cdot h = \frac{1}{2} v^2$ \quad $\Rightarrow$ \quad $h = \frac{v^2}{2g} = \frac{(4\,\frac{\text{m}}{\text{s}})^2}{2 \cdot 10\,\frac{\text{m}}{\text{s}^2}} = \frac{16}{20}\,\text{m} = 0{,}8\,\text{m}$}

\textbf{Ergebnis:} $h = $ \lsg{0,8 m}
\end{teil}
...
\lsg{$\eta = \frac{P_\text{Licht}}{P_\text{zu}} = \frac{4\,\text{W}}{10\,\text{W}} = 0{,}4$}

\textbf{Ergebnis:} $\eta = $ \lsg{0,4} $= $ \lsg{40} \%
...
\vspace{6pt}
\textbf{Ergebnis:} $P_\text{Wärme} = $ \lsg{$10\,\text{W} - 4\,\text{W} = 6\,\text{W}$}
\end{teil}
...
\lsg{Florian hat die Formel für v falsch umgestellt. Die richtige Umstellung lautet:}

\lsg{$E_\text{kin} = \frac{1}{2} m \cdot v^2$ \quad $\Rightarrow$ \quad $v = \sqrt{\frac{2 \cdot E_\text{kin}}{m}}$}

\lsg{Er hat vergessen, durch die Masse zu teilen und mit 2 zu multiplizieren.}

\item \operator{Berechne} die richtige Geschwindigkeit.

\lsg{$v = \sqrt{\frac{2 \cdot 100\,\text{J}}{2\,\text{kg}}} = \sqrt{\frac{200}{2}\,\frac{\text{m}^2}{\text{s}^2}} = \sqrt{100\,\frac{\text{m}^2}{\text{s}^2}} = 10\,\frac{\text{m}}{\text{s}}$}

\textbf{Ergebnis:} $v = $ \lsg{10 m/s}

\textit{Hinweis: Florians Endergebnis ist zufällig richtig, aber sein Rechenweg ist falsch!}
\end{teil}
...
\vspace{4pt}
\textbf{Rechnung:} $\eta_\text{ges} = \eta_1 \cdot \eta_2 = $ \lsg{$0{,}40 \cdot 0{,}90 = 0{,}36$ (36\%)}
...
\lsg{$E_\text{el} = E_\text{Wind} \cdot \eta_\text{ges} = 1000\,\text{J} \cdot 0{,}36 = 360\,\text{J}$}

\textbf{Ergebnis:} $E_\text{el} = $ \lsg{360 J}
\end{teil}
...
\vfill
\begin{center}
\small\textcolor{loesung}{\textbf{Gesamtpunkte: \maxpunkte{} / \maxpunkte{} BE}}
\end{center}
