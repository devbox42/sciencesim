...
Ein \textbf{Kraftpfeil} zeigt drei Eigenschaften: \luecke{2.2cm}, \luecke{2.2cm}, \luecke{2.2cm}
...
a) Zeichne $F_1$ (blau), $F_2$ (grün) und $F_{\text{res}}$ (rot) ein:
...
b) Berechnung: $F_{\text{res}} = $ \luecke{5cm} $= $ \luecke{2cm}
...
a) Zeichne beide Kräfte und die Resultierende ein:
...
b) $F_{\text{res}} = $ \luecke{5cm} $= $ \luecke{2cm}

c) Welches Team gewinnt? \luecke{3cm}
...
a) $F_{\text{res}} = $ \luecke{4cm}

\vspace{4mm}

b) Was passiert? \luecke{8cm}
