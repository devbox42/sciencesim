...
\begin{tikzpicture}[scale=1.0]
    ...
    \foreach \x in {2,4,6,8} {
        \node[below, loesung] at (\x,-0.15) {\small \x};
    }
    \node[below, loesung] at (10,-0.15) {\scriptsize $\cdot 10^{14}$ Hz};
    \foreach \y in {-2,-1,1,2} {
        \node[left, loesung] at (-0.1,\y) {\small \y};
    }
    \node[left, loesung] at (-0.1,-3) {\scriptsize eV};
    ...
    \fill[loesung] (0,-2.3) circle (3pt);
    \node[loesung, right] at (0.1,-2.3) {\small $-W_A$};
...
$f_G = $ \lsg{5,5 -- 5,6} $\cdot 10^{14}$ Hz

\vspace{0.3em}
Physikalische Bedeutung: \lsg{Grenzfrequenz -- unterhalb kein Photoeffekt}
...
\lsg{$0 = h \cdot f_G - W_A \quad \Rightarrow \quad W_A = h \cdot f_G$}
...
\vspace{0.3em}
Was bedeutet dieser negative Wert physikalisch? \lsg{$n = -W_A$ (negative Austrittsarbeit)}

\vspace{0.6em}

\item \textbf{Geradengleichung:} Formuliere die vollständige Gleichung. \hfill (2 BE)

$E_{\text{kin}} = $ \lsg{$h$} $\cdot f - $ \lsg{$W_A$} \quad \lsg{(= Einsteinsche Photogleichung)}
