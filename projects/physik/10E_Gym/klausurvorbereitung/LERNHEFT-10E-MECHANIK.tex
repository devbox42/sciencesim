...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\color{gray}Lernheft Mechanik -- Klasse 10E}
...
\centering
\vspace*{2cm}
{\Huge\bfseries\color{physikblue} Mechanik\par}
\vspace{0.5cm}
{\LARGE Klausurvorbereitung Klasse 10E\par}
\vspace{0.3cm}
{\large Gymnasium -- Schuljahr 2024/25\par}
\vspace{1.5cm}

\begin{tcolorbox}[colback=lightblue, colframe=physikblue, width=12cm, boxrule=1pt]
\centering\large\bfseries
Themen dieser Klausur:
\begin{itemize}[leftmargin=1.5cm]
\item Gleichförmige Bewegung
\item Beschleunigte Bewegung
\item Zusammengesetzte Bewegungen
\item Freier Fall
\item Luftwiderstand \& Grenzgeschwindigkeit \textit{(nur qualitativ)}
\end{itemize}
\end{tcolorbox}
...
\begin{tikzpicture}
\node[rectangle, rounded corners=3pt, fill=merkegreen!20, draw=merkegreen, line width=1pt, minimum width=10cm, minimum height=1cm] {
  \textbf{Merke:} $g \approx 10\,\mathrm{m/s^2}$ (immer kopfrechenbar!)
};
\end{tikzpicture}

\vfill
{\small Greenhouse Schools Rostock\par}
\end{titlepage}
...
\vspace{0.3cm}
\textbf{Kopfrechenbare Werte (auswendig lernen!):}
\begin{center}
...
\textbf{0-100 in X Sekunden} -- Autotests geben immer die Beschleunigung an. Ein Tesla Model S schafft $a \approx 4\,\mathrm{m/s^2}$!
\end{alltag}
...
\textbf{Nützliche Zusatzformel} (ohne Zeit):
\[
...
\begin{alltag}
\textbf{Handy fallen lassen?} In 0,5 Sekunden knallt es auf den Boden -- aus nur 1,25 m Höhe!\\[0.3em]
...
\begin{tipp}
In der Klausur \textbf{immer} $g = 10\,\mathrm{m/s^2}$ verwenden -- das macht kopfrechnen möglich!
\end{tipp}
...
\hline
Fallbeschleunigung & $g = 10\,\mathrm{m/s^2}$ \\
\hline
...
\begin{tipp}
\textbf{Diese Werte auswendig können:}
\begin{multicols}{2}
...
\begin{itemize}
\item[\textbf{$\star$}] Grundlagen -- Einsetzen und berechnen
\item[\textbf{$\star\star$}] Fortgeschritten -- Umstellen und mehrere Schritte
\item[\textbf{$\star\star\star$}] Anspruchsvoll -- Transfer und Begründen
\end{itemize}
\end{tcolorbox}

\vspace{0.5cm}

\begin{aufgabe}[$\star$ Aufgabe A: Grundlagen]
...
\begin{aufgabe}[$\star\star$ Aufgabe B: Fortgeschritten]
...
\begin{aufgabe}[$\star\star\star$ Aufgabe C: Transfer und Begründen (AFB III)]
...
\begin{aufgabe}[$\star\star\star$ Aufgabe D: Komplexe Analyse]
...
\item \textbf{$g = 9{,}81$ statt $g = 10$} -- Kopfrechnen unmöglich!
