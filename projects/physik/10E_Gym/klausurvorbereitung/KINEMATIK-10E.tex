...
\begin{tcolorbox}[colback=physikblue!5, colframe=physikblue, boxrule=1pt, title=\textbf{Kinematik und Dynamik: Was ist der Unterschied?}]
...
\textbf{In dieser Klausur:} Wir bleiben bei der \textbf{Kinematik}, also der Beschreibung der Bewegung.
\end{tcolorbox}
...
\textbf{Führerschein:} Der Reaktionsweg wird mit $v \cdot t_{\text{Reaktion}}$ berechnet. Das ist gleichförmige Bewegung!
\end{alltag}

\subsection{Definition und Grundformel}

Eine Bewegung heißt \textbf{gleichförmig}, wenn die Geschwindigkeit \textbf{konstant} bleibt. Der Körper legt dann in gleichen Zeitabschnitten gleiche Strecken zurück.
...
Bei doppelter Geschwindigkeit: \textbf{4-facher Bremsweg}. Das kann Leben retten!\\[0.3em]
...
\textbf{Nützliche Zusatzformel} (ohne Zeit, auch \textbf{„zeitlose Formel"} genannt):
\[
...
\begin{tcolorbox}[colback=merkegreen!5, colframe=merkegreen, boxrule=0.5pt]
\textbf{Erkennst du das Muster?} Der freie Fall ist einfach eine beschleunigte Bewegung mit $a = g$:

\begin{center}
\renewcommand{\arraystretch}{1.4}
\begin{tabular}{|l|c|c|}
\hline
\rowcolor{merkegreen!20}
& \textbf{Allgemein} & \textbf{Freier Fall} \\
\hline
Beschleunigung & $a$ & $g \approx 10\,\mathrm{m/s^2}$ \\
\hline
Geschwindigkeit & $v = a \cdot t$ & $v = g \cdot t$ \\
\hline
Weg & $s = \frac{1}{2} a \cdot t^2$ & $s = \frac{1}{2} g \cdot t^2 = 5t^2$ \\
\hline
Zeitlos & $v^2 = 2as$ & $v^2 = 2gs$ \\
\hline
\end{tabular}
\end{center}
Beim senkrechten Wurf nach oben wirkt $g$ bremsend, also $a = -g$.
\end{tcolorbox}
...
Erreichte über 1000 km/h. \textbf{Warum so schnell?}\\
...
\textbf{c)} Zwei Kugeln aus dem gleichen Material, eine davon mit doppeltem Durchmesser. Welche hat die höhere Grenzgeschwindigkeit? Begründe.
\end{aufgabe}
...
\textbf{Hinweis:} Diese Aufgaben trainieren das \textbf{Lesen und Interpretieren} von Bewegungsdiagrammen. Das ist eine typische Klausuraufgabe! Nimm dir Zeit und arbeite systematisch.
\end{tcolorbox}
...
\item \textbf{Fläche und Steigung verwechselt:} Im $v(t)$-Diagramm ist Fläche = Weg!
\end{enumerate}
\end{warnung}
