...
% ==================== RECHTWINKLIG ====================
\textbf{Rechtwinklige Kräfte (Pythagoras)} \abmark{5}

\vspace{2mm}
\begin{center}
\fbox{\large $\Fres = \sqrt{\Fone^2 + \Ftwo^2}$}
\end{center}

\begin{center}
\begin{tikzpicture}[scale=0.4]
    \draw[gridcolor, very thin] (0,0) grid (8,10);
    \xkreuz{1}{1}
    \draw[f1color, thick, ->] (1,1) -- (7,1);
    \node[below, font=\scriptsize, f1color] at (4,0.5) {$F_1$ = 3 N};
    \draw[f2color, thick, ->] (1,1) -- (1,9);
    \node[left, font=\scriptsize, f2color] at (0.5,5) {$F_2$ = 4 N};
    \draw[frescolor, ultra thick, ->] (1,1) -- (7,9);
    \node[above right, font=\scriptsize, frescolor] at (4.3,5.3) {$F_{\text{res}}$ = 5 N};
    \draw[black, dashed, thin] (7,1) -- (7,9);
    \draw[black, dashed, thin] (1,9) -- (7,9);
\end{tikzpicture}
\end{center}

$\Fres = \sqrt{3^2 + 4^2} = \sqrt{25} = 5\,\text{N}$
...
\hline
Senkrecht & $\Fres = \sqrt{\Fone^2 + \Ftwo^2}$ & Pythagoras (3-4-5) \\
\hline
\end{tabular}
\end{center}
