...
\textbf{Kraftpfeil:} \lsg{Angriffspunkt}, \lsg{Richtung}, \lsg{Betrag}
...
b) Berechnung: $F_{\text{res}} = $ \lsg{3 N + 4 N} $= $ \lsg{7 N} \hfill (2 P)
...
b) $F_{\text{res}} = $ \lsg{6 N $-$ 4 N} $= $ \lsg{2 N nach links} \hfill (2 P)

c) Welches Team gewinnt? \lsg{Team A} \hfill (1 P)
...
a) $F_{\text{res}} = $ \lsg{5 N $-$ 5 N = 0 N} \hfill (1 P)

\vspace{4mm}

b) Was passiert? \lsg{Nichts, Gleichgewicht (keine Bewegung)} \hfill (1 P)
...
\begin{tikzpicture}[scale=0.5]
    ...
    \xkreuz{2}{2}
    % Lösungen
    \draw[f1color, very thick, ->] (2,2) -- (8,2);
    \node[f1color, below, font=\tiny] at (5,2) {$F_1$ = 3 N};
    \draw[f2color, very thick, ->] (2,2) -- (2,10);
    \node[f2color, left, font=\tiny] at (2,6) {$F_2$ = 4 N};
    \draw[frescolor, ultra thick, ->] (2,2) -- (8,10);
    \node[frescolor, above right, font=\small] at (5,6) {$F_{\text{res}}$ = 5 N};
    % Parallelogramm
    \draw[frescolor, dashed, thin] (8,2) -- (8,10);
    \draw[frescolor, dashed, thin] (2,10) -- (8,10);
\end{tikzpicture}
\end{center}

b) Berechne mit Pythagoras: \hfill (2 P)

$F_{\text{res}} = \sqrt{F_1^2 + F_2^2} = \sqrt{3^2 + 4^2} = \sqrt{9 + 16} = \sqrt{25} = $ \lsg{5 N}
